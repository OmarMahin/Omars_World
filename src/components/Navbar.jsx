		...
		<nav ref={navbarRef}>
			<Container>
				<div
					...
					className={`fixed lg:absolute left-0 w-full z-30 duration-300  ${
						showNavShadow && !showNavMenu
							? "drop-shadow-md pb-4 bg-backgroundColor"
							: "drop-shadow-none pb-0 bg-transparent"
					}  lg:pb-0 lg:drop-shadow-none`}
				>
					<Flex
						className={`relative left-0 ml-3 mt-[20px] gap-2 items-center lg:hidden  cursor-pointer duration-300 ${
							!showNavMenu ? "top-0" : "top-[-60px]"
						}`}
					>
						<FaBars
							className=' w-5 h-5'
						...
						/>
						<h2
							className=' text-mainFont text-fontColor font-bold text-[21px]'
							onClick={changeNavState}
						>
							Menu
						</h2>
					...
					<Flex className={"justify-center"}>
						<List
							className={
								showNavMenu
									? `fixed top-0 left-[0] lg:flex lg:static px-5 pt-6 w-[220px] bg-[#161616] opacity-[0.97] lg:bg-transparent h-[100vh] lg:h-auto lg:gap-7 lg:p-0 lg:w-auto duration-300`
