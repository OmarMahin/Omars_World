...
function botBody(w, h, d, px, py, pz, m) {
    ...
    botBody_.quaternion.setFromEuler(0, (-Math.PI * 2.9) / 4, 0);
                ...
                if (
                    ...
                    if (
                        ...
                        controlBot(
                            ...
                            "S",
                            speed,
                            robot.getWheelSpeed(1),
                            robot.getWheelSpeed(0)
                        );
                    }
                }
            }
...
function botMovement(target, robot) {
    ...
    if (targetDistance3 > 5 && rotate) {
        ...
        if (
            Math.abs(targetDistance1 - targetDistance2) <= distanceDiffThreshold && !forward
    ...
    if (forward && !rotate) {
        ...
        if (
            (Math.abs(targetDistance1 - targetDistance2) > distanceDiffThreshold ||
            targetDistance3 - targetDistance2 < -1) && targetDistance3 > 5
        ) {
            controlBot(robot, "S", speed, robot.getWheelSpeed(1), robot.getWheelSpeed(0));
            if (
                Math.abs(robot.getWheelSpeed(0)) < 0.01 &&
        ...
        if (destinationReached && !initialBotCoordsReached) {
            if (botBodyShape.position.distanceTo(target.position) < 1.5 ) {
                console.log("Reached");
                controlBot(robot, "S", speed, robot.getWheelSpeed(1), robot.getWheelSpeed(0));
                forward = false;
                rotate = false;
